...
# removing full datasets to save some RAM
rm(blogtext)
rm(twittext)
rm(newstext)
```
...
```{r}
sum(frequencylist$frequency >=17)
sum(frequencylist$frequency[1:4515]) / sum(frequencylist$frequency)
```

Now we're at 75%, with only 4,515 words out of the total of 33444 words of our dataset. That means that with only 13% of the words we can recreate 75% of the source documents.
...
```{r}
keeplist <- frequencylist$feature[1:4515]
...
n <- sum(frequencylist_2G$frequency >= 3)
...
n <- sum(frequencylist_3G$frequency >= 1)
