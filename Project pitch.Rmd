...
75% of the words in our dataset appear 4 times or less. Let's see the proportion of the data made of words with a frequency of 100 or more :
...
```{r}
sum(frequencylist$frequency[1:811]) / sum(frequencylist$frequency)
```
With only these 811 words, we can recreate almost 45% of the whole dataset. That's not quite enough, we'd like to be close to 75% :

```{r}
sum(frequencylist$frequency >=20)
sum(frequencylist$frequency[1:4072]) / sum(frequencylist$frequency)
```

Now we're at 86%, with only 4,072 words out of the total of 33444 words of our dataset. That means that with only 12% of the words we can recreate 86% of the source documents.
...
```{r}
keeplist <- frequencylist$feature[1:4072]
...
Therefore, what we will now do is take our tokens list and make it into 2-grams and 3-grams token lists, only keep as subset of combinations that appear at least twice, and build three models combined into one in the following way :
...
```{r, cache=T}
train_tokens_2G <- tokens_ngrams(train_tokens, n = 2, concatenator = " ")
traindfm_2G <- dfm(train_tokens_2G, tolower = F)
frequencylist_2G <- textstat_frequency(traindfm_2G)
n <- sum(frequencylist_2G$frequency >= 2)
keeplist_2G <- frequencylist_2G$feature[1:n]
model_tokens_2G <- tokens_select(train_tokens_2G, keeplist_2G, selection = "keep")
```

### Building the 3-grams token list 

```{r, cache=T}
train_tokens_3G <- tokens_ngrams(train_tokens, n = 3, concatenator = " ")
traindfm_3G <- dfm(train_tokens_3G, tolower = F)
frequencylist_3G <- textstat_frequency(traindfm_3G)
n <- sum(frequencylist_3G$frequency >= 2)
keeplist_3G <- frequencylist_3G$feature[1:n]
model_tokens_3G <- tokens_select(train_tokens_3G, keeplist_3G, selection = "keep")
```
